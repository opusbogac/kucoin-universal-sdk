...
async function checkAvailableBalance(accountService: AccountService, price: number, amount: number,
                                     marketType: MarketType): Promise<boolean> {
    /**
     * Checks if the available balance is sufficient for the trade.
     *
     * @param accountService - The account API client instance.
...
async function getLastTradedPrice(spotService: SpotService, futuresService: FuturesService): Promise<[number, number]> {
    /**
...
async function addSpotOrderWaitFill(
    spotService: SpotService,
    symbol: string,
    side: MarketSide,
    amount: number,
...
async function addFuturesOrderWaitFill(
    futuresService: FuturesService,
    symbol: string,
    side: MarketSide,
    amount: number,
...
async function addMarginOrderWaitFill(
    marginService: MarginService,
    symbol: string,
    amount: number,
    price: number
...
async function main() {
    ...
    const futuresService: FuturesService = kucoinRestService.getFuturesService();
    const spotService: SpotService = kucoinRestService.getSpotService();
    const marginService: MarginService = kucoinRestService.getMarginService();
    const accountService: AccountService = kucoinRestService.getAccountService();
